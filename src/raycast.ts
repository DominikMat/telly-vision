...
let raycastVisibilityData: Array<Array<Point>> = new Array(raycastAngularResolution)
let raycastOriginX = -100
let raycastOriginY = -100

export function raycastFromPosition(x:number, y:number, apartment: Apartment) {
    raycastVisibilityData = new Array(raycastAngularResolution)
    apartment.resetVisibilityData()
    if (!apartment.positionWithinApartmentBounds(x,y)) return;
...
export function drawRaycast(ctx: CanvasRenderingContext2D) {
    if (!ctx) return;
...
export function drawRaycastOrigin(ctx: CanvasRenderingContext2D) {
    if (!ctx) return;
