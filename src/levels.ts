...
export class Level {
    ...
    maxAvailableReflectionObject: ReflectionObjectType = ReflectionObjectType.Mirror
    tellyPos: Point
    minScore: number

    constructor (pathPoints: Array<Point>, maxAvailableReflectionObject: ReflectionObjectType, tellyPos: Point, pathControlPoints: Array<Point> = [], minScore: number = defaultMinimumScore) {
    ...
    drawCleaningPath(ctx: CanvasRenderingContext2D, apartment: Apartment) {
        if (!ctx || this.pathPoints.length == 0) return;
        ...
        for (let i=1; i<this.pathPoints.length; i++) {
            let p = this.pathPoints[i]; if (!p) continue
            p = apartment.uvToWorld(p)

            if ((i-1) < this.pathControlPoints.length) {
                let cp = this.pathControlPoints[i-1]; if (!cp) continue
    ...
    getRaycastPosition(progress: number, apartment: Apartment): Point {
        ...
        if (pathIdx == this.pathPoints.length-1) return this.pathPoints[pathIdx] ? this.pathPoints[pathIdx] : new Point()

        // get left and right points
        let p1 = this.pathPoints[pathIdx]; let p2 = this.pathPoints[pathIdx+1]
        if (!p1 || !p2) return new Point()
        p1 = apartment.uvToWorld(p1); p2 = apartment.uvToWorld(p2)
        ...
        // quadratic curve - triple lerp
            ...
            let cp = this.pathControlPoints[pathIdx]; if (!cp) return new Point()
            cp = apartment.uvToWorld(cp)
...
export class LevelManager {
    ...
    getCurrentLevel(): Level | null | undefined { 
        if (this.selectedLevel < this.levels.length) return this.levels[this.selectedLevel]
        else return null
    }
    getCurrentLevelIdx(): number { return this.selectedLevel }
    unlockNextLevel() {
    ...
    changeCurrentLevel(dir: number): Level | undefined {
        if (dir == 1 && this.isNextLevelUnlocked()) this.selectedLevel += 1
        if (dir == -1 && this.selectedLevel > 0) this.selectedLevel -= 1
